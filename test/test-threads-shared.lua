...
for _, tensorType in ipairs({"Float", "Double"}) do
  local x = {}
  ...
  for i=1,njob do
     x[i] = torch[tensorType .. 'Tensor']():ones(D)
     xh[i] = torch[tensorType .. 'Tensor']():ones(D)
     xs[i] = torch[tensorType .. 'Storage'](D):fill(1)
     for j=1,K do
        z[(i-1)*K+j] = "blah" .. i .. j
     end
  end
  ...
  for i=1,njob do
     pool:addjob(
        function()
           assert(x[i]:sum() == D)
           assert(xh[i]:sum() == D)
           assert(torch[tensorType .. 'Tensor'](xs[i]):sum() == D)
           for j=1,K do
              assert(z[(i-1)*K+j] == "blah" .. i .. j)
           end
           x[i]:add(1)
           xh[i]:add(1)
           torch[tensorType .. 'Tensor'](xs[i]):add(1)
  ...
  for i=1,njob do
     assert(x[i]:sum() == 2*D)
     assert(xh[i]:sum() == 2*D)
     assert(torch[tensorType .. 'Tensor'](xs[i]):sum() == 2*D)
  end
  ...
  for i=1,njob do
     assert(y[i]:sum() == 2*D)
     assert(yh[i]:sum() == 2*D)
     assert(torch[tensorType .. 'Tensor'](ys[i]):sum() == 2*D)
  end
